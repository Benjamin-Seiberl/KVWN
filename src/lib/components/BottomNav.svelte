...
<!-- Backdrop -->
{#if subtabMenuOpen}
	<button class="pnav-backdrop" onclick={() => subtabMenuOpen = false} aria-label="Schließen"></button>
{/if}

<!-- Subtab Strip -->
<div
	class="pnav-strip"
	class:pnav-strip--open={subtabMenuOpen && navVisible}
	class:pnav-strip--nav-hidden={!navVisible}
>
	<div class="pnav-strip-inner">
		{#each visibleSubtabs as st}
			{@const active = $currentSubtab === st.key}
			<button
				class="pnav-strip-item"
				class:active
				onclick={() => selectSubtab(st.key)}
			>
				<span class="material-symbols-outlined">{st.icon}</span>
				<span class="pnav-strip-item-label">{st.label}</span>
				{#if active}<div class="pnav-strip-item-dot"></div>{/if}
			</button>
		{/each}
	</div>
</div>
